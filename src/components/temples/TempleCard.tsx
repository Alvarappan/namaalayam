...
export default function TempleCard({ temple, index }: Props) {
  return (
    <Link
    ...
    >
      <div className={`relative h-44 bg-gradient-to-br ${templeGradients[index % templeGradients.length]} overflow-hidden`}>
      ...
      <div className="p-4">
        ...
        <p className="text-stone-400/80 text-xs leading-relaxed line-clamp-2">{temple.description}</p>
        <div className="flex flex-wrap gap-1.5 mt-3">
          {temple.tags.slice(0, 2).map((tag) => (
            <span key={tag} className="px-2 py-0.5 bg-stone-700/60 text-stone-400 text-xs rounded-md">{tag}</span>
          ))}
        </div>
      </div>
    </Link>
